...
erase_black_margin(Image &image)
{
    ...
    auto get_color = [pixels, width](unsigned int i, unsigned int j){
    ...
    auto set_color = [pixels, width, &white](unsigned int i, unsigned int j){
        __typeof__(pixels) ptr = pixels + j * width + i;
		ptr->red = white.redQuantum();
		ptr->blue = white.blueQuantum();
		ptr->green = white.greenQuantum();
    };
		...
		unsigned int threshold, unsigned int w_limit, unsigned int h_limit)
{
    ...
    auto get_color = [pixels, width](unsigned int i, unsigned int j){
        __typeof__(pixels) ptr = pixels + j * width + i;
		auto r = ptr->red & 0xff;
		auto g = ptr->green & 0xff;
		auto b = ptr->blue & 0xff;
